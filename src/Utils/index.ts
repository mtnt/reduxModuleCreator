...
export function combineReducers<T extends {
...
export interface RMCActionCreator<A extends RegularRMCAction = RegularRMCAction, R = void> {
  (...args: A['creator'] extends (...args: any) => infer R ? Parameters<A['creator']> : []): (A['creator'] extends (...args: any) => any ? ReturnType<A['creator']>: {}) & {type: string};
...
export type ReducerActionsType<A extends Record<string, RMCAction>> = {
  [AN in keyof A]: A[AN] extends RegularRMCAction
    ? A[AN]["creator"] extends (...args: any) => any
      ...
      : { type: string }
    : A[AN] extends ProxyAction ? ReturnType<A[AN]["proxy"]> & { type: string } : never
}
